...
bool IsBetweenSamsHouse(int position, int s, int t);

void countApplesAndOranges(int s, int t, int a, int b, vector<int> apples, vector<int> oranges) {
    int applesInsideSamsHouse = 0;
    int orangesInsideSamsHouse = 0;
    
    for (int i = 0; i < apples.size(); i++) {
        int applePosition = a + apples[i];
        if(IsBetweenSamsHouse(applePosition, s, t)){
    ...
    for (int i = 0; i < oranges.size(); i++) {
        ...
        if(IsBetweenSamsHouse(orangePosition, s, t)){
            orangesInsideSamsHouse++;
        }
    }
    cout << applesInsideSamsHouse << endl << orangesInsideSamsHouse << endl;
}
